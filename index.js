...
app.post('/api/mahasiswa',upload.single('images'),(req, res) => {
    ...
    if (!req.file) {
        ...
        const querySql = 'INSERT INTO mahasiswa (nim,nama,tanggal_lahir,alamat,prodi) values (?,?,?,?,?,?);';
         
        // jalankan query
        koneksi.query(querySql,[ nim,nama, tanggal_lahir,alamat,prodi], (err, rows, field) => {
            // error handling
            if (err) {
    ...
    const data = { ...req.body };
    const querySql = 'INSERT INTO mahasiswa (nim,nama,tanggal_lahir,alamat,foto,prodi) values (?,?,?,?,?,?);';
 
// jalankan query
koneksi.query(querySql,[ nim,nama, tanggal_lahir,alamat,foto,prodi], (err, rows, field) => {
    // error handling
    if (err) {
        return res.status(500).json({ message: 'Gagal insert data!', error: err });
    }
...
app.put('/api/mahasiswa/:nim', (req, res) => {
    ...
    const alamat = req.body.alamat;
    const prodi = req.body.prodi;
    const queryUpdate = 'UPDATE mahasiswa SET nama=?,tanggal_lahir=?,alamat=?,prodi=? WHERE nim = ?';
    ...
    koneksi.query(querySearch, req.params.nim, (err, rows, field) => {
        ...
        if (rows.length) {
            ...
            koneksi.query(queryUpdate, [nama,tanggal_lahir,alamat,prodi, req.params.nim], (err, rows, field) => {
                // error handling
                if (err) {
                    return res.status(500).json({ message: 'Ada kesalahan', error: err });
                }
